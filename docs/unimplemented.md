...
| **전시 latency·운영** | 로컬: `build_queue_blocking` ~331s → 768 frames, dequeue 즉시. IG 유지 | Railway free tier에서 refill lag·cold start 허용 여부, `n_steps`/배치 추가 튜닝 필요 시 일정 |
...
**로컬 벤치** (CPU): `build_queue_blocking` 331s, queue 768 frames, `build_frame_from_queue` 즉시.
